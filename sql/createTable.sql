CREATE EXTERNAL TABLE IF NOT EXISTS dwh.web_logs (
  `DateTime` STRING,
  `EdgeLocation` STRING,
  `ScBytes` INTEGER,
  `CIp` STRING,
  ...
  `ScRangeStart` STRING,
  `ScRangeEnd` STRING
) PARTITIONED BY (`month` string) STORED AS PARQUET LOCATION 's3://logs.l1x.be/dwh/web-logs/' -- MSCK REPAIR TABLE dwh.web_logs ;
